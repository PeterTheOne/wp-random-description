...
Vagrant.configure(2) do |config|
  ...
  config.vm.provision "shell", inline: <<-SHELL
    ...
    sudo -E wp core install --url=localhost:8080 --title=wp-random-description --admin_user=admin --admin_password=password --admin_email=fake@fake.fake --skip-email --path=/var/www/html --allow-root

    sudo rm index.html
    sudo ln -s /vagrant/ /var/www/html/wp-content/plugins/wp-random-description

    sudo -E wp plugin activate wp-random-description --allow-root
  SHELL
end